...
本章では、宇宙線研究の成立から現在の観測までをたどり、エネルギースペクトルと質量組成、到来方向、粒子の加速と伝搬を説明する。

\section{宇宙線研究の成立}
20 世紀初頭、密閉容器内に蓄えた電荷の減少から、空気のわずかな電離が知られていた。検電器は、放電の速さから容器内の電離率を求める装置である。
当時は、地殻中の放射性物質が放つ \(\gamma\) 線が電離の主因と考えられていた。この仮説に従えば、地殻から離れるほど電離率は低下する。
Theodor Wulf は 1910 年、エッフェル塔上で地上の 87\% に相当する電離率を測定した。高度約 \(300\ \mathrm{m}\) は当時の見積もりで大気中の約 4 吸収長に相当し、放射線が地殻だけから来るなら、電離率は地上の数パーセントまで低下するはずであった~\cite{Carlson2012}。
Domenico Pacini は 1911 年、水面下約 \(3\ \mathrm{m}\) で電離率が水面上より約 20\% 低下することを観測した。水による遮蔽で電離率が下がったことから、電離を生じさせる放射線の一部は上方から入射すると結論した~\cite{Carlson2012,HessTranslation2018}。

Victor Franz Hess は 1912 年に 7 回の気球飛行を行い、内部の圧力を一定に保つ密閉型の Wulf 検電器で高度ごとの電離率を測定した。
第 7 回飛行の測定には、電離箱を厚さ \(3\ \mathrm{mm}\) の金属壁で囲った 2 台を用いた。\(\alpha\) 線と \(\beta\) 線はこの壁を透過できないため、検電器は主として透過力の強い放射線に応答した~\cite{Hess1912,HessTranslation2018}。
...
部分日食中に行われた第 1 回飛行では、太陽が月に隠されても電離率は低下せず、夜間の飛行にも昼間との明確な差はなかった。Hess はこの結果から、太陽を放射線の直接の放射源から除外した。
最高高度 \(5350\ \mathrm{m}\) に達した第 7 回飛行では、高度とともに電離率が増加する傾向が明確となり、Hess は強い透過力をもつ放射線が上方から大気へ入射していると結論した~\cite{Hess1912,HessTranslation2018}。
この観測は宇宙線の発見とされている。Hess はその業績により、1936 年のノーベル物理学賞を受賞した~\cite{Carlson2012,NobelHess1936}。
...
この透過性放射を宇宙起源の高周波電磁波と考えた Robert Andrews Millikan は、1925 年の \textit{Science} 論文で cosmic rays という名称を初めて用いた~\cite{Millikan1925,Carlson2012}。
...
検出器信号から一次エネルギーを求める較正には、実験ごとに十数パーセントの系統的不確かさがある。報告される折れ曲がりのエネルギーには、統計誤差に加えて、このエネルギースケールの不確かさが含まれる~\cite{AugerTAEnergyScale2019,TALESpectrum2018}。

\subsection{質量組成}\label{subsec:cosmic_ray_composition}
質量組成は、エネルギースペクトルとは異なる観測量から、宇宙線の起源と伝搬を制約する。
質量組成を 1 つの量で表す場合には、\(\ln A\) の平均
\begin{equation}
  \langle \ln A\rangle
\end{equation}
...
図~\ref{fig:tale_lna}は、TALE ハイブリッド検出器による \(10^{16.5}\)--\(10^{18.5}\ \mathrm{eV}\) の測定結果を、Auger などの高統計測定と比較している~\cite{TALEMassComposition2026}。

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.85\hsize]{fig/TALE-Hybrid-lnA.pdf}
  \caption{宇宙線の \(\langle\ln A\rangle\)。TALE ハイブリッド質量組成測定の Fig.~22 より引用~\cite{TALEMassComposition2026}。}
  \label{fig:tale_lna}
\end{figure}
...
\begin{table}[htb]
  ...
  \begin{tabular}{@{}p{0.18\hsize}p{0.19\hsize}p{0.25\hsize}p{0.28\hsize}@{}}
    ...
    \hline
    KASCADE-Grande & \(10^{16}\)--\(10^{18}\ \mathrm{eV}\) & シンチレータ、ミューオン検出器 & 電子数とミューオン数の比で試料を分け、軽い原子核と重い原子核のスペクトルを測定 \\
...
図~\ref{fig:hillas_plot}は、代表的な候補天体の大きさと磁場強度を 2 軸に取り、式~\eqref{eq:hillas_condition}の閉じ込め条件を示す Hillas 図である。図中の横軸 \(R\) は本稿の \(L\) に相当する。
一定の最大エネルギーに対応する \(BL=\mathrm{const.}\) の線は、両軸を対数で表すと傾き \(-1\) の直線になる。特定のエネルギーまで陽子を加速できる天体は、その直線より \(BL\) の大きい側に分布する。
鉄原子核では \(Z=26\) であるため、同じ \(B\) と \(L\) でも陽子の約 26 倍のエネルギーまで閉じ込められる。ただし、この図が示すのは閉じ込めの必要条件であり、実際の最大エネルギーは加速時間やエネルギー損失によっても制限される。
...
質量組成の推定では、ハドロン相互作用モデルへの依存とミューオンパズルが主要な系統要因となる。
...
KASCADE-Grande のような解析は、電子・ミューオン比を用いて試料を軽い原子核と重い原子核に対応する群へ分類する。シミュレーションがミューオン数を過小評価すれば、データから推定される質量組成は実際より重くなる。また、組成に依存する地表信号の補正を通じて、エネルギースケールも変化しうる。
このため、second knee 付近の質量群別スペクトルの解釈は、相互作用モデルだけでなく、\(X_{\max}\)、地表電磁成分、異なるしきい値で測定したミューオン成分との整合性にも依存する。
...
\end{equation}
がよく用いられる。
通常の GH 関数では、パラメータ \(X_0\) と放射長 \(X_0\) に同じ記号を用いる。本稿では両者を区別するため、前者を \(X_0^{\mathrm{GH}}\) と表す。定義域は \(X_{\max}>X_0^{\mathrm{GH}}\)、\(\Lambda>0\) とする。
...
この方法では、望遠鏡の視線方向と光の到来時刻からシャワー軸の位置と方向を決める。次に、大気中での吸収・散乱とチェレンコフ光の寄与を補正し、縦方向プロファイルを再構成する。
...
\end{equation}
と表される。
後述する NKG 型関数と AGASA の経験式は、全粒子数そのものではなく、地上の粒子密度分布 \(\rho(r)\) を記述する~\cite{KamataNishimura1958,Greisen1960,Yoshida1994}。
...
この違いは、後述するシャワー曲面の構造にも影響する~\cite{Cazon2004}。
